...
const deleteGrocery = id => {
  const removeIdex = groceries.value.findIndex (grocery => grocery.id)
  groceries.value.splice(removeIndex, l)
...
<template>
  ...
  <form class="newGroceryForm" @submit.prevent="addGrocery">
    ...
    <ul>
      <li v-for="grocery in groceries" @click="deleteGrocery"> {{ newGrocery }}</li>
      {{ grocery.name }}
    </ul>
</main>
<pre>{{ groceries }}</pre>
...
main {
  ...
  .title {
    @apply m-2 text-6xl font-ligt tracking-wider text-accent;
  }
...
ul {
  ...
  li {
    ...
    &:hover {
    @apply bg-purplish front-bold cursor-pointer;
    }
  }
}
</style>
